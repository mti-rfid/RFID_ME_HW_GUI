...
#include <stdio.h>
#include "../inc/win_qextserialport.h"
...
*/
Win_QextSerialPort::Win_QextSerialPort(QObject* parent)
	:QextSerialBase(parent)
{
...
*/
Win_QextSerialPort::Win_QextSerialPort(const QString & name, QObject* parent)
	:QextSerialBase(name, parent)
{
    Win_Handle=INVALID_HANDLE_VALUE;
}
...
*/
Win_QextSerialPort::Win_QextSerialPort(const PortSettings& settings, QObject* parent)
	: QextSerialBase(parent)
{
    Win_Handle=INVALID_HANDLE_VALUE;
...
*/
Win_QextSerialPort::Win_QextSerialPort(const QString & name, const PortSettings& settings, QObject* parent)
	: QextSerialBase(parent)
{
    Win_Handle=INVALID_HANDLE_VALUE;
    setPortName(name);
...
bool Win_QextSerialPort::open(OpenMode mode) {
    ...
    if (!isOpen()) {
        ...
        /*open the port*/
			...
			port.append(":");		
			wchar_t we[7];
			port.toWCharArray(we);
			LPTSTR COM = we;
			Win_Handle=CreateFile(COM, GENERIC_READ|GENERIC_WRITE, FILE_SHARE_READ|FILE_SHARE_WRITE, NULL, OPEN_EXISTING, 0, NULL);
		#else			
			Win_Handle=CreateFileA(port.toAscii(), GENERIC_READ|GENERIC_WRITE, FILE_SHARE_READ|FILE_SHARE_WRITE, NULL, OPEN_EXISTING, 0, NULL);
		#endif
