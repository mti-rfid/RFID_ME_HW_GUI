...
{	
	...
	*iLength = 0;
	...
	if( tmpLen > m_HidCaps.InputReportByteLength)	
		return USB_REPORT_COUNT_NOT_MATCH;
...
bool ClsUSBPort::OpenEndpoint()
{
	...
	//Get the HID Handle Point
	_stprintf_s(DeviceName, _T("%s\\%s"), m_pstrcDetail->DevicePath, _T("PIPE01") );	 
	...
	//Get the HID Handle Point 
	_stprintf_s(DeviceName, _T("%s\\%s"), m_pstrcDetail->DevicePath, _T("PIPE02") );	
...
char* ClsUSBPort::API_AskUSBName()
{
#ifdef	TRY_DEBUG
	OutputDebugString( _T("Transfer API_AskUSBName\n") );
#endif
	...
	return (char *)m_strPath.GetString();
}
