...
void Win_QextSerialPort::ungetChar(char c) {
	...
	// to prevent warning C4100: 'c' : unreferenced formal parameter
	Q_UNUSED(c);
...
/*!
\fn bool Win_QextSerialPort::waitForReadyRead(int msecs)
Waits max. for msecs milliseconds, if no data received returns false, else true.
*/
bool Win_QextSerialPort::waitForReadyRead(int msecs)
{
	int count;

	for(count = 0; count < msecs; count++)
	{
		Sleep(1);
