	...
	, QrfeTraceModule("CTagViewManager")
{
	...
	//m_treeWidget->setHeaderLabels(QStringList() << "Reader" << "Hardware" << "Software" << "Action" << "State" );//mod by rick 
	m_treeWidget->setHeaderLabels(QStringList() << "Reader" << "Model Name" << "Serial Number" << "Action" << "State" );
	//End by yingwei tseng for modifying GUI style, 2010/08/30
...
void CTagViewManager::readerInfoWrite() {
	...
	foreach(QString rId, m_readerEntries.keys())
	{
		...
		m_readerInfoFile->write("Reading Speed = " + QByteArray::number(m_readRateCalc->getReadRate(rId), 'g', 6) + "\r\n");
