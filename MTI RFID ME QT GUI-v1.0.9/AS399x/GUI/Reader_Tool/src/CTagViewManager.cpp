...
//Add by yingwei tseng for print tagid and read rate to readerInfo.txt, 2010/06/21 
void CTagViewManager::readerInfoWrite(int position, const QString msg)
{
	static QMutex mutex;
	mutex.lock();

	if (m_readerInfoFile->size() > TRC_FILE_SIZE)
	{
		m_readerInfoFile->close();
		m_readerInfoFile->setFileName(
	...
	if (position)
		m_readerInfoFile->write(msg.toAscii() + '\n');
	else
		m_readerInfoFile->write(msg.toAscii() + "     ");
...
void CTagViewManager::updateReaderInfo(QString readerId)
{
	...
	//Add by yingwei tseng for print tagid and read rate to readerInfo.txt, 2010/06/21 
	readerInfoWrite(1, QString::number(m_readRateCalc->getReadRate(readerId)));
